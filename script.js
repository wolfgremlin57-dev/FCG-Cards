...
const aliasMap = {
    ...
    OfficeLR: ["OfficeLR", "Office", "Uber", "Omni"],
    Right_Office: ["Right_Office", "OfficeLR", "Office", "Right_Side", "Uber", "Omni"],
    Left_Office: ["Left_Office", "OfficeLR", "Office", "Left_Side", "Uber", "Omni"],
    Opponent: ["Opponent", "Opponent Front", "Opponent Omni", "Opponent Vent", "Omni/Opponent Omni"],
    "Off Map": ["Off Map"],
};
...
function toJpNumber(j) {
    ...
    return parseInt(String(j).replace(/[()]/g, ""), 10) || 0;
}
...
function render() {
    ...
    const end = Math.min(start + cardsPerPage, filteredCards.length);
...
<div class="card" data-rarity="${rarityAttr}">
    ...
    <div class="card-body" hidden>
        ...
        ${card.power ? `<p>Power: ${card.power.replace(/_Null/g, "Null").replace(/H100/g, "100%").replace(/10G/g, "10%").replace(/0W/g, "0%").replace(/20/g, "20%").replace(/30/g, "30%").replace(/40/g, "40%").replace(/50/g, "50%").replace(/60/g, "60%").replace(/70/g, "70%").replace(/80/g, "80%").replace(/90/g, "90%")}</p>` : ""}
...
`;
    ...
    if (stats) {
        stats.textContent = `${filteredCards.length} card(s) shown`;
    }
...
function goToPage(page) {
    const totalPages = Math.max(1, Math.ceil(filteredCards.length / cardsPerPage));
    const target = Math.min(Math.max(1, Math.round(Number(page) || 1)), totalPages);
...
function renderPagination() {
    ...
    for (let i = startPage; i <= endPage; i++) {
        pagination.appendChild(makeBtn(i, () => go(i), i === currentPage));
    }
...
function update() {
    ...
    if (typeinclusivity === "Exact") {
        ...
        if (type) {
            filteredCards = filteredCards.filter(c => c.type && c.type.includes(type));
        }
    ...
    if (design) {
        // tokens split on commas AND slashes (data uses "/")
        let designValues = design.split(/[,/]/).map(s => s.trim()).filter(Boolean);
        // "Gator" option covers both Gator and Alligator designs
        if (designValues.includes("Gator")) designValues.push("Alligator");
        if (designinclusivity === "Exact") {
            filteredCards = filteredCards.filter(c => c.design === design);
        } else if (designinclusivity === "Inclusive") {
            filteredCards = filteredCards.filter(c => c.design && designValues.some(v => c.design.split("/").map(s => s.trim()).includes(v)));
        } else if (designinclusivity === "Exclusive") {
            filteredCards = filteredCards.filter(c => c.design && !designValues.some(v => c.design.split("/").map(s => s.trim()).includes(v)));
        }
    }
    ...
    if (map) {
        ...
        if (mapinclusivity === "Exact") {
            ...
            filteredCards = filteredCards.filter(c => c.Map && mapValues.some(v => c.Map.split("/").map(s => s.trim()).includes(v)));
    ...
    if (sorting === "Alphabetically") {
        ...
        filteredCards.sort((a, b) => toPowerNumber(a.power) - toPowerNumber(b.power));
    } else if (sorting === "Power Reversed") {
        filteredCards.sort((a, b) => toPowerNumber(b.power) - toPowerNumber(a.power));
    } else if (sorting === "JP") {
        filteredCards.sort((a, b) => toJpNumber(a.Jp) - toJpNumber(b.Jp));
    } else if (sorting === "JP Reversed") {
        filteredCards.sort((a, b) => toJpNumber(b.Jp) - toJpNumber(a.Jp));
    }
...
document.getElementById("topPrev").addEventListener("click", () => goToPage(currentPage - 1));
document.getElementById("topNext").addEventListener("click", () => goToPage(currentPage + 1));
document.getElementById("topFirst").addEventListener("click", () => goToPage(1));
document.getElementById("topLast").addEventListener("click", () => {
    goToPage(Math.ceil(filteredCards.length / cardsPerPage));
});
document.getElementById("pageJump").addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        goToPage(document.getElementById("pageJump").value);
    }
});
document.getElementById("pageJump").addEventListener("blur", () => {
    goToPage(document.getElementById("pageJump").value);
});
...
function setFiltersVisible(visible) {
    ...
    toggleFilters.innerHTML = visible
        ? "&#9660; Hide Filters"
        : "&#9658; Show Filters";
    try {
        localStorage.setItem("fcgFiltersVisible", visible ? "1" : "0");
    } catch (e) { /* ignore */ }
}

toggleFilters.addEventListener("click", () => {
    setFiltersVisible(controlsPanel.classList.contains("collapsed"));
});

let saved = null;
try { saved = localStorage.getItem("fcgFiltersVisible"); } catch (e) { /* ignore */ }
if (saved === "0") {
    setFiltersVisible(false);
} else if (saved === "1") {
    setFiltersVisible(true);
} else {
    // No saved preference: collapse by default on small/phone screens
